...
public class EditCusView extends JDialog implements ActionListener{
	...
	public EditCusView(DBConnector db, KhachHang nv) {
		this.db = db;
		...
		if (e.getSource() == btnThem) {
			...
			if(checkFormat() == true){
				try {
					...
					db.editCustomer(tfID.getText(), tfHoTen.getText(), date,
							tfDiachi.getText(), tfSDT.getText(), nv.getCoin(),tfUsername.getText(),tfPassword.getText());
					dispose();
